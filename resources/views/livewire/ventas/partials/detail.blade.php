		...
		<div class="card simple-title-task ui-sortable-handle">
			<div class="card-body">
					...
					<table class="table table-bordered table-striped mt-1">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<input type="number" id="r{{$item-id}}" wire:change="updateQty({{$item->id}}, $('#r' + {{$item->id}}).val())" style="font-size: 1rem!important" class="form-control" value="{{$item->quantity}}">
								</td>
								<td class="text-center">
									<h6>${{number_format($item->price * $item->quantity,2)}}</h6>
								</td>
								<td class="text-center">
									<button onclick="confirm('{{$item->id}}', 'removeItem', '¿Está Seguro de Querer Eliminar El Registro Seleccionado?')" class="btn btn-dark mbmobile">
				...
				<div wire:loading.inline wire:target="saveSale">
					<h4 class="text-danger text-center">Guardando los Registros de la Venta...</h4>
				</div>
			</div>
		</div>
